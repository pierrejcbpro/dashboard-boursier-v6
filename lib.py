...
"""
v7.1 — Library (lib.py)
Cohérente avec Synthèse Flash, Portefeuille et Recherche Universelle.
- Ajout : variations pct_1d / pct_7d / pct_30d
- Correction : fetch_all_markets normalisé et robuste
- Analyse IA et news intégrées
"""
...
# -------------------------------------------------------------------
# 🧮 Calcul des indicateurs techniques + variations
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# 🌍 Agrégation multi-indices (correctif complet)
# -------------------------------------------------------------------
def members(index_name:str):
    ...
    if not url: return pd.DataFrame(columns=["ticker","name","index"])
    try:
        ...
        out=df[[tcol,ncol]].copy(); out.columns=["ticker","name"]
...
def fetch_all_markets(markets, days_hist=90):
    ...
    for idx,_ in markets:
        ...
        for col in ["pct_1d","pct_7d","pct_30d"]:
            if col not in met.columns: met[col]=np.nan
    ...
    keep=["Indice","Ticker","name","Close","MA20","MA50","ATR14",
