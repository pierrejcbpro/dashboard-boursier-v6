# -*- coding: utf-8 -*-
"""
v6.9 — Mon Portefeuille (benchmark + IA + répartition + volatilité + proximité adaptative)
...
- Colonne Proximité (%) + Signal Entrée (🟢/⚠️/🔴), surbrillance adaptative clair/sombre
- Camembert de répartition
- Graphique de performance vs Benchmark (CAC / DAX / S&P500 / NASDAQ100)
"""
...
# Choix période (graphique) + benchmark
...
# Colonne Proximité (%) + Signal Entrée
def proximity_info(row):
    entry = row.get("Entrée (€)")
    px = row.get("Cours (€)")
    if not (np.isfinite(entry) and np.isfinite(px)) or entry == 0:
        return np.nan, ""
    prox = ((px / entry) - 1) * 100
    if abs(prox) <= 2:
        emoji = "🟢"
...
out[["Proximité (%)", "Signal Entrée"]] = out.apply(
    lambda r: proximity_info(r), axis=1, result_type="expand"
)

# Couleurs pour la décision IA
...
# Rendu tableau avec styles : surbrillance proximité + couleurs
st.dataframe(
    out.style
      .apply(lambda row: highlight_near_entry_adaptive(row, col="Proximité (%)"), axis=1)
...
tot_pct = (tot_gain / (tot_val - tot_gain) * 100) if (tot_val - tot_gain) != 0 else 0.0
...
if hist_graph.empty or "Date" not in hist_graph.columns:
    ...
    if df_lines:
        ...
        if not bmk.empty and not tot.empty:
            ...
            base = (
                full.groupby("Type")
                    .apply(lambda g: g.assign(Pct=(g["Valeur"]/g["Valeur"].iloc[0]-1)*100))
            ...
            st.caption("Benchmark indisponible pour cette période.")
    else:
        st.caption("Aucune ligne exploitable pour le graphique.")
